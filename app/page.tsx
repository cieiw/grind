      ...
      <div className="operation-layout">
        ...
        <section className="operation-workspace">
          ...
          {operationView === "routine" ? <div className="routine-view">
            ...
            <div className="today-routines">{data.accounts.map((account) => { const time = timeline(account); const items = accountRoutine(account); return <section key={account.id}><header><strong>{account.name}</strong><span>{time.periodType === "week" ? `Semana ${time.periodNumber}` : `Mês ${time.periodNumber}`} · dia {time.dayNumber}</span></header>{[...items].sort((a, b) => Number(account.completedRoutine.includes(a.id)) - Number(account.completedRoutine.includes(b.id))).map((item) => <label key={item.id}><input type="checkbox" checked={account.completedRoutine.includes(item.id)} onChange={() => { const completed = account.completedRoutine.includes(item.id) ? account.completedRoutine.filter((id) => id !== item.id) : [...account.completedRoutine,item.id]; updateAccount(account.id,{completedRoutine:completed,posts:item.trackPosts && !account.completedRoutine.includes(item.id) ? account.posts+1 : item.trackPosts && account.completedRoutine.includes(item.id) ? Math.max(0,account.posts-1):account.posts}); }}/><span className="check">✓</span><span><strong>{item.title}</strong><small>{item.description}</small></span></label>)}{!items.length ? <p>Sem etapas para hoje.</p> : null}</section>; })}</div>
          </div> : null}
        </section>
      </div>
    </div> : null}
