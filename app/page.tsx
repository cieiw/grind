...
function pendingFirst<T extends { done: boolean }>(items: T[]) {
...
export default function Home() {
  ...
  function openVisibleInstagram() {
    overviewAccounts.forEach((account) => {
      if (!account.instagramUrl.trim()) return;
      const link = document.createElement("a");
      link.href = account.instagramUrl;
      link.target = "_blank";
  ...
  useEffect(() => { const frame = requestAnimationFrame(() => { if (summaryRef.current) resizeSummary(summaryRef.current); }); return () => cancelAnimationFrame(frame); }, [data.summaries, selectedDate, mainView]);
  ...
  return <main className="app-shell" style={themeStyle}>
    ...
    {mainView === "today" ? <div className="today-view">
      ...
      <div className="daily-grid">
        <section className="panel habits-panel">
          ...
          <DndContext sensors={sensors} onDragEnd={reorderHabits}><SortableContext items={data.habits.map((habit) => habit.id)} strategy={verticalListSortingStrategy}><div className="habit-list">{[...data.habits].sort((a, b) => Number(checkedHabits.includes(a.id)) - Number(checkedHabits.includes(b.id))).map((habit) => <SortableItem id={habit.id} className={`habit-row ${checkedHabits.includes(habit.id) ? "checked" : ""}`} key={habit.id}>{habitHandle => <><button className="drag-handle" {...habitHandle.attributes} {...habitHandle.listeners} aria-label="Arrastar hábito">⠿</button><input id={`habit-${habit.id}`} type="checkbox" checked={checkedHabits.includes(habit.id)} onChange={() => setData((current) => { const currentChecks = current.habitChecks[selectedDate] ?? []; const nextChecks = currentChecks.includes(habit.id) ? currentChecks.filter((id) => id !== habit.id) : [...currentChecks, habit.id]; return { ...current, habits: [...current.habits].sort((a, b) => Number(nextChecks.includes(a.id)) - Number(nextChecks.includes(b.id))), habitChecks: { ...current.habitChecks, [selectedDate]: nextChecks } }; })}/><label htmlFor={`habit-${habit.id}`} className="check">✓</label><span>{habit.label}</span><small>{habitStreak(habit.id)}d</small><button onClick={() => setData((current) => ({ ...current, habits: current.habits.filter((item) => item.id !== habit.id) }))}>×</button></>}</SortableItem>)}</div></SortableContext></DndContext>
    ...
    {mainView === "operation" ? <div className="operation-view">
      ...
      <div className={`operation-layout ${operationView === "routine" ? "routine-config-layout" : ""}`}>
        {operationView !== "routine" ? <aside className="account-rail">
          ...
          <div className="account-list">{filteredAccounts.map((account) => { const time = timeline(account); const routine = accountRoutine(account); const done = routine.filter((item) => account.completedRoutine.includes(item.id)).length; const included = !overviewAccountIds.length || overviewAccountIds.includes(account.id); return <div className={`account-list-item ${included ? "included" : ""}`} key={account.id}><button className={selectedAccount?.id === account.id ? "active" : ""} onClick={() => { setSelectedAccountId(account.id); setOverviewAccountIds((current) => current.length === 0 ? [account.id] : current.includes(account.id) ? current.filter((id) => id !== account.id) : [...current, account.id]); }}><strong>{account.name}</strong><span>{time.periodType === "week" ? `Semana ${time.periodNumber}` : `Mês ${time.periodNumber}`} · dia {time.dayNumber}</span><i><b style={{ width: `${routine.length ? done / routine.length * 100 : 0}%` }}/></i></button></div>; })}{!filteredAccounts.length ? <p className="empty-state">Adicione o primeiro perfil.</p> : null}</div>
        </aside> : null}
        ...
        <section className="operation-workspace">
          ...
          {operationView === "material" ? <div className="material-view">
            ...
            <div className="material-list">{filteredAccounts.map((account) => { const status = materialStatus(account, materialKind); const scope = materialKind === "photos" ? account.photoMaterialScope ?? 30 : account.materialScope; return <article className={materialSelection.includes(account.id) ? "selected" : ""} key={account.id} onClick={() => setMaterialSelection((current) => current.includes(account.id) ? current.filter((id) => id !== account.id) : [...current,account.id])}><div><strong>{account.name}</strong><span>{account.batch == null ? "Sem lote" : `Lote ${account.batch}`}</span></div><label onClick={(event) => event.stopPropagation()}>Dias<input type="number" min="0" value={status.remaining} onChange={(event) => updateAccount(account.id, materialKind === "photos" ? {photoMaterialDays:Number(event.target.value),photoMaterialUpdatedAt:new Date().toISOString()} : {materialDays:Number(event.target.value),materialUpdatedAt:new Date().toISOString()})}/></label><label onClick={(event) => event.stopPropagation()}>Escopo<input type="number" min="1" value={scope} onChange={(event) => updateAccount(account.id, materialKind === "photos" ? {photoMaterialScope:Number(event.target.value)} : {materialScope:Number(event.target.value)})}/></label><div className="material-bar"><span>{status.remaining}/{scope} dias</span><i><b style={{width:`${status.percent}%`}}/></i></div></article>; })}</div>
          </div> : null}
