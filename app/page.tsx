    ...
    {mainView === "operation" ? <div className="operation-view">
      ...
      <div className="operation-layout">
        <aside className="account-rail">
          ...
          <div className="account-list">{filteredAccounts.map((account) => { const time = timeline(account); const routine = accountRoutine(account); const done = routine.filter((item) => account.completedRoutine.includes(item.id)).length; return <div className="account-list-item" key={account.id}><input type="checkbox" checked={overviewAccountIds.includes(account.id)} onChange={() => setOverviewAccountIds((current) => current.includes(account.id) ? current.filter((id) => id !== account.id) : [...current, account.id])} aria-label={`Mostrar ${account.name} no Geral`}/><button className={selectedAccount?.id === account.id ? "active" : ""} onClick={() => setSelectedAccountId(account.id)}><strong>{account.name}</strong><span>{time.periodType === "week" ? `Semana ${time.periodNumber}` : `Mês ${time.periodNumber}`} · dia {time.dayNumber}</span><i><b style={{ width: `${routine.length ? done / routine.length * 100 : 0}%` }}/></i></button></div>; })}{!filteredAccounts.length ? <p className="empty-state">Adicione o primeiro perfil.</p> : null}</div>
        </aside>
